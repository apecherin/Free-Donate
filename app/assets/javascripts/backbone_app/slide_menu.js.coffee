class Views.SlideMenu extends Backbone.Marionette.Layout
  template: 'slide_menu'
  ...
  events:
    'click #slide-menu #login' : 'closeMenu'
    'click #slide-menu .signup' : 'closeMenu'
    'click #slide-menu li a' : 'closeMenu'
  ...
  resizeWin: ->
    MyApp.vent.trigger 'change_widht_of_screen'
    false

  searchItems: ->
    @fetchSearchResults()
    @closeMenu()
    @$('#slide-menu input[name=query]').val('')

  fetchSearchResults: ->
    input_value = @$('#slide-menu input[name=query]').val()
    results = new Collections.SearchResults
    results.fetch data: {query: @$('#slide-menu input[name=query]').val()}, success: (results) =>
