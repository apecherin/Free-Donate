class Fragments.Expenses.PurchaseInfo extends Backbone.Marionette.ItemView
  template: 'fragments/vehicles/expenses/purchase_info'
  ...
  onRender: ->
    @currentCurrencyRate = if @vehiclePrice.get('purchase_price')? then Store.get('current_currency_rate') else @vehiclePrice.get('currency_rate')[Store.get('currentCurrency').toLowerCase()]
    if @modeEdit
      @$('.seller_id').chosen()
    else
#      @setOdometerValue('.purchase_odometer', @vehiclePrice.get('purchase_odometer'))
