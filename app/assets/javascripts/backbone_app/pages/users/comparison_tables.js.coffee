...
class Pages.Users.ComparisonTables extends Backbone.Marionette.Layout
  ...
  getTitle: ->
    if @user && @currentUser?.id is @user.id
      I18n.t('my_comparison_tables_title', scope: 'ui')
    else if @user
      I18n.t('comparison_tables_title_for_name', scope: 'ui', name: @user.get('username'))
    else
      I18n.t('comparison_tables_recent_title', scope: 'ui')

  serializeData: ->
    title: @getTitle()
    bradcumView: @bradcumView
    searchView: @searchView
    countTables: @comparisonTables.size()
